...
app.controller('itemCatController', function($scope, $controller, baseService){
    ...
    //分模块查询
    $scope.findModules = function (parentId) {
        /** 设置父级id */
        $scope.parentId = parentId;
        baseService.sendGet("/itemCat/findModule" ,"parentId=" + parentId ).then(function (response) {
    ...
    $scope.saveOrUpdate = function(){
        ...
        baseService.sendPost("/itemCat/" + url, $scope.entity)
            .then(function(response){
                if (response.data){
                    /** 重新加载数据 */
                    $scope.findModules($scope.parentId);
                    $scope.entity = null;
                }else{
                    alert("操作失败！");
                }
